...
/*   Updated: 2026/03/02 18:22:45 by nsantand         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*aux_checknumber(char *str, char *argv)
{
	...
	if (!check_spaces(argv, str))
		return (NULL);
	if (argv[0] == '\0')
		return (ft_putendl_fd("Error: Empty argument", 2), free(str), NULL);
	if (!argv)
		return (ft_putendl_fd("Error: Argument not int", 2), free(str), NULL);
